    ...
    return (
        <Dialog.Root>
            <Dialog.Trigger className='rounded-md text-left flex flex-col bg-slate-800 p-5 gap-3 overflow-hidden relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400 outline-none'>
                <span className="text-sm  font-medium text-slate-300">{formatDistanceToNow(note.date)}</span>
            ...
            <Dialog.Content className='fixed inset-0 md:inset-auto overflow-hidden md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none'>
                ...
                <button 
                  ...
                  className='w-full bg-slate-800 py-4 text-center text-sm text-slate-300 outline-none font-medium group'>
                    <span className='text-red-400 hover:underline group-hover:underline'>Delete</span> this note?
                </button>
            </Dialog.Content>
        </Dialog.Portal>
        </Dialog.Root>
